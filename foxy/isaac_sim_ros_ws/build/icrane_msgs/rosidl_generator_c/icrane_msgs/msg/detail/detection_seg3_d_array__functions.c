// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from icrane_msgs:msg/DetectionSeg3DArray.idl
// generated code does not contain a copyright notice
#include "icrane_msgs/msg/detail/detection_seg3_d_array__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"
// Member `detections`
#include "icrane_msgs/msg/detail/detection_seg3_d__functions.h"

bool
icrane_msgs__msg__DetectionSeg3DArray__init(icrane_msgs__msg__DetectionSeg3DArray * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    icrane_msgs__msg__DetectionSeg3DArray__fini(msg);
    return false;
  }
  // detections
  if (!icrane_msgs__msg__DetectionSeg3D__Sequence__init(&msg->detections, 0)) {
    icrane_msgs__msg__DetectionSeg3DArray__fini(msg);
    return false;
  }
  return true;
}

void
icrane_msgs__msg__DetectionSeg3DArray__fini(icrane_msgs__msg__DetectionSeg3DArray * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // detections
  icrane_msgs__msg__DetectionSeg3D__Sequence__fini(&msg->detections);
}

bool
icrane_msgs__msg__DetectionSeg3DArray__are_equal(const icrane_msgs__msg__DetectionSeg3DArray * lhs, const icrane_msgs__msg__DetectionSeg3DArray * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // detections
  if (!icrane_msgs__msg__DetectionSeg3D__Sequence__are_equal(
      &(lhs->detections), &(rhs->detections)))
  {
    return false;
  }
  return true;
}

bool
icrane_msgs__msg__DetectionSeg3DArray__copy(
  const icrane_msgs__msg__DetectionSeg3DArray * input,
  icrane_msgs__msg__DetectionSeg3DArray * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // detections
  if (!icrane_msgs__msg__DetectionSeg3D__Sequence__copy(
      &(input->detections), &(output->detections)))
  {
    return false;
  }
  return true;
}

icrane_msgs__msg__DetectionSeg3DArray *
icrane_msgs__msg__DetectionSeg3DArray__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  icrane_msgs__msg__DetectionSeg3DArray * msg = (icrane_msgs__msg__DetectionSeg3DArray *)allocator.allocate(sizeof(icrane_msgs__msg__DetectionSeg3DArray), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(icrane_msgs__msg__DetectionSeg3DArray));
  bool success = icrane_msgs__msg__DetectionSeg3DArray__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
icrane_msgs__msg__DetectionSeg3DArray__destroy(icrane_msgs__msg__DetectionSeg3DArray * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    icrane_msgs__msg__DetectionSeg3DArray__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
icrane_msgs__msg__DetectionSeg3DArray__Sequence__init(icrane_msgs__msg__DetectionSeg3DArray__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  icrane_msgs__msg__DetectionSeg3DArray * data = NULL;

  if (size) {
    data = (icrane_msgs__msg__DetectionSeg3DArray *)allocator.zero_allocate(size, sizeof(icrane_msgs__msg__DetectionSeg3DArray), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = icrane_msgs__msg__DetectionSeg3DArray__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        icrane_msgs__msg__DetectionSeg3DArray__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
icrane_msgs__msg__DetectionSeg3DArray__Sequence__fini(icrane_msgs__msg__DetectionSeg3DArray__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      icrane_msgs__msg__DetectionSeg3DArray__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

icrane_msgs__msg__DetectionSeg3DArray__Sequence *
icrane_msgs__msg__DetectionSeg3DArray__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  icrane_msgs__msg__DetectionSeg3DArray__Sequence * array = (icrane_msgs__msg__DetectionSeg3DArray__Sequence *)allocator.allocate(sizeof(icrane_msgs__msg__DetectionSeg3DArray__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = icrane_msgs__msg__DetectionSeg3DArray__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
icrane_msgs__msg__DetectionSeg3DArray__Sequence__destroy(icrane_msgs__msg__DetectionSeg3DArray__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    icrane_msgs__msg__DetectionSeg3DArray__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
icrane_msgs__msg__DetectionSeg3DArray__Sequence__are_equal(const icrane_msgs__msg__DetectionSeg3DArray__Sequence * lhs, const icrane_msgs__msg__DetectionSeg3DArray__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!icrane_msgs__msg__DetectionSeg3DArray__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
icrane_msgs__msg__DetectionSeg3DArray__Sequence__copy(
  const icrane_msgs__msg__DetectionSeg3DArray__Sequence * input,
  icrane_msgs__msg__DetectionSeg3DArray__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(icrane_msgs__msg__DetectionSeg3DArray);
    icrane_msgs__msg__DetectionSeg3DArray * data =
      (icrane_msgs__msg__DetectionSeg3DArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!icrane_msgs__msg__DetectionSeg3DArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          icrane_msgs__msg__DetectionSeg3DArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!icrane_msgs__msg__DetectionSeg3DArray__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
