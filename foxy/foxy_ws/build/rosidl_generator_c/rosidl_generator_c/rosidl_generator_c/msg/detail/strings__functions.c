  ...
  rosidl_generator_c__msg__Strings__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rosidl_generator_c__msg__Strings * data =
      (rosidl_generator_c__msg__Strings *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rosidl_generator_c__msg__Strings__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rosidl_generator_c__msg__Strings__fini(&data[i]);
        }
        free(data);
        return false;
      }
