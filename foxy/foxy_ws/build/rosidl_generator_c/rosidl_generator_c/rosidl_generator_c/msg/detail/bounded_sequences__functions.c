  ...
  rosidl_generator_c__msg__BoundedSequences__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rosidl_generator_c__msg__BoundedSequences * data =
      (rosidl_generator_c__msg__BoundedSequences *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rosidl_generator_c__msg__BoundedSequences__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rosidl_generator_c__msg__BoundedSequences__fini(&data[i]);
        }
        free(data);
        return false;
      }
