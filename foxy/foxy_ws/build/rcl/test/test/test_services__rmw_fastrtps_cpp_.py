...
def generate_test_description():
    ...
    launch_description.add_action(
        ExecuteProcess(
            cmd=['/workspace/foxy_ws/build/rcl/test/service_fixture__rmw_fastrtps_cpp'],
            name='service_fixture',
        )
    )

    executable_under_test = ExecuteProcess(
        cmd=['/workspace/foxy_ws/build/rcl/test/client_fixture__rmw_fastrtps_cpp', 'service_fixture'],
        name='client_fixture',
    )
