  ...
  void * untyped_ros_message)
{
  ...
  _BuiltinTypeSequencesIdl__ros_msg_type * ros_message = static_cast<_BuiltinTypeSequencesIdl__ros_msg_type *>(untyped_ros_message);
  // Field name: char_sequence_unbounded
  {
    uint32_t cdrSize;
    cdr >> cdrSize;
    ...
    if (!rosidl_runtime_c__char__Sequence__init(&ros_message->char_sequence_unbounded, size)) {
      return "failed to create array for field 'char_sequence_unbounded'";
    }
    auto array_ptr = ros_message->char_sequence_unbounded.data;
...
{
  return max_serialized_size_rosidl_generator_py__msg__BuiltinTypeSequencesIdl(
    full_bounded, 0);
}
